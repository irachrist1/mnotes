...
export default function Roadmap() {
  ...
  return (
    <section id="roadmap" ref={ref} className="py-24 bg-white dark:bg-stone-950">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <p className="text-blue-600 dark:text-blue-500 text-sm font-medium tracking-wide uppercase">Roadmap</p>
        ...
        >
          {phases.map((phase) => (
            <motion.div
              key={phase.phase}
              variants={fadeUpVariants}
              className="bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-white/[0.06] rounded-xl p-5"
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <p className="text-xs text-stone-400 dark:text-stone-500 font-mono">{phase.phase}</p>
                  <h3 className="text-base font-semibold text-stone-900 dark:text-white">{phase.title}</h3>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className={`w-1.5 h-1.5 rounded-full ${phase.statusColor}`} />
                  <span className="text-xs text-stone-500 dark:text-stone-400">{phase.status}</span>
                </div>
              </div>
              <ul className="space-y-1.5">
                {phase.items.map((item) => (
                  <li key={item} className="flex items-center gap-2 text-sm text-stone-500 dark:text-stone-400">
                    <svg className="w-3.5 h-3.5 text-stone-300 dark:text-stone-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
