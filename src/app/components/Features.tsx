...
const features = [
    ...
    preview: (
      <div className="mt-4 space-y-2">
        {[
          ...
          <div key={s.name} className="flex items-center justify-between bg-gray-50 dark:bg-gray-800/50 rounded-lg px-3 py-2 border border-gray-100 dark:border-white/[0.04]">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-sky-500 dark:text-sky-400 tabular-nums">{s.rating}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 bg-gradient-to-br from-sky-500/10 to-cyan-500/5 rounded-lg p-3 border border-sky-500/10">
        <div className="flex items-start gap-2">
          <div className="w-6 h-6 bg-sky-500/20 rounded flex items-center justify-center flex-shrink-0 mt-0.5">
            <svg className="w-3.5 h-3.5 text-sky-500 dark:text-sky-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <div>
            <p className="text-xs text-gray-700 dark:text-gray-300">Your consulting work has the best revenue-to-time ratio at $92/hr. Product revenue is growing slower but requires less active time.</p>
            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1.5">92% confidence / gemini-2.5-flash</p>
          </div>
        </div>
      </div>
    ),
    ...
    preview: (
      <div className="mt-4 grid grid-cols-2 gap-2">
        {[
          ...
          <div key={s.label} className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-2 border border-gray-100 dark:border-white/[0.04]">
            <p className="text-[10px] text-gray-500">{s.label}</p>
            <p className="text-sm font-semibold text-gray-900 dark:text-white tabular-nums">{s.value}</p>
  ...
  },
  {
    ...
    preview: (
      <div className="mt-4 space-y-2">
        <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-800/50 rounded-lg px-3 py-2 border border-gray-100 dark:border-white/[0.04]">
          <span className="text-[10px] text-gray-500">Provider</span>
          <span className="text-xs text-gray-700 dark:text-gray-300">OpenRouter</span>
        </div>
        <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-800/50 rounded-lg px-3 py-2 border border-gray-100 dark:border-white/[0.04]">
          <span className="text-[10px] text-gray-500">Model</span>
          <span className="text-xs text-gray-700 dark:text-gray-300">gemini-2.5-flash</span>
        </div>
      </div>
    ),
  },
...
export default function Features() {
  ...
  return (
    <section id="features" ref={ref} className="py-24 bg-white dark:bg-gray-950 relative">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <p className="text-sky-500 dark:text-sky-400 text-sm font-medium tracking-wide uppercase">Features</p>
        ...
        >
          {features.map((feature) => (
            <motion.div
              key={feature.title}
              variants={fadeUpVariants}
              className={`group relative bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-white/[0.06] rounded-xl p-5 ${feature.border} transition-all duration-300 overflow-hidden ${feature.span}`}
            >
